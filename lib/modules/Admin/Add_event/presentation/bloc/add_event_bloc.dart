...
class AddEventBloc extends Bloc<AddEventEvent, AddEventState> {
  ...
  final TextEditingController themedescriptionController =TextEditingController();
  final TextEditingController themedecorationController =TextEditingController();
  final TextEditingController themeactivitiesController =TextEditingController();
  ...
  FutureOr<void>onSubmitAddEvent(OnAddEvent event, Emitter<AddEventState> emit)async {
    emit(AddEventLoadingState());
    if(event.eventInfo.title.isEmpty || event.eventInfo.imageUrl.isEmpty ||
    event.eventInfo.description.isEmpty || event.eventInfo.service.isEmpty ||
    event.eventInfo.price.isEmpty || event.eventInfo.package.isEmpty ||
  ...
  FutureOr<void>onSubmitAddTheme(OnAddTheme event, Emitter<AddEventState> emit)async {
    emit(AddEventLoadingState());
    if(event.themeInfo.theme.isEmpty || event.themeInfo.themetitle.isEmpty ||
    event.themeInfo.themeimage.isEmpty || event.themeInfo.themedescription.isEmpty ||
    event.themeInfo.themedecoration.isEmpty || event.themeInfo.themeactivities.isEmpty ){
      emit(AddEventErrorState(error: 'All Fields are required!'));
    }try{
